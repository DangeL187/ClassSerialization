...
class Archive {
 ...
 private:
  ...
  void serialize_basic(T& basic, const std::string& name) {
    if (_os) {
      *_os << std::string(_tags.size() * 4, ' ') << "<" << name << ">" << basic
           << "</" << name << ">\n";
      return;
    }

    std::string value =
        _nodes.top()->FirstChildElement(name.c_str())->GetText();
  ...
  void serialize_pointer(T*& ptr, const std::string& name) {
    setParent(name);
    if (_os) {
      *_os << std::string(_tags.size() * 4, ' ') << "<is_null>" << !ptr
           << "</is_null>\n";
  ...
  template <typename T>
  void serialize_shared_pointer(std::shared_ptr<T>& shared_ptr,
  ...
  void serialize_string(std::string& string, const std::string& name) {
    if (_os) {
      *_os << std::string(_tags.size() * 4, ' ') << "<" << name << ">" << string
           << "</" << name << ">\n";
      return;
    }
  ...
  void serialize_vector(std::vector<T>& vector, const std::string& name) {
    setParent(name);
    if (_os) {
      *_os << std::string(_tags.size() * 4, ' ') << "<size>" << vector.size()
           << "</size>\n";
      ...
      size_t size =
          std::stoull(_nodes.top()->FirstChildElement("size")->GetText());
      vector.resize(size);
      for (int i = 0; i < vector.size(); i++) {
        connect(vector[i], "item-" + std::to_string(i));
      }
